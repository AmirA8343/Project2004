...
import { NUTRITION_JSON_SCHEMA } from "./prompts";
import { runPizzaScript } from "./pizza";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { description = "", photoUrl, language = "en" } = req.body || {};
...
${NUTRITION_JSON_SCHEMA}`;
    ...
    const responseBody = {
      ...
      ai_foods: normalizeAiFoods(simpleParsed.ai_foods, displayName),
    };
...
`;
  ...
  if (!description && stage1.foods && stage1.foods.length === 1) {
    ...
    // try to grab "330 ml", "130 g", etc. from summary / name / description
...
${NUTRITION_JSON_SCHEMA}`;
    ...
    const imageBypassMessages = [
      ...
      { role: "user", content: `Food to analyze: ${qtyLabel}` },
    ];
    ...
    const imageSimpleRaw = imageSimpleData.choices?.[0]?.message?.content || "";
    ...
    if (!imageSimpleParsed) {
      ...
      const displayName = qtyLabel || onlyFood.name || description || "Food";
  ...
  if (combinedText.includes("pizza")) {
    ...
    try {
      const pizzaResult = await runPizzaScript(
        OPENAI_API_KEY,
        photoUrl,
        description,
        language
